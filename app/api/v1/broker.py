...
async def connect_kotak_full(creds: KotakCredentials, current_user: dict = Depends(get_current_user)):
    try:
        ...
        # 🟢 ASLI FIX: Token seedha Kotak ke Response se nikal rahe hain!
        token = None
        if isinstance(resp2, dict) and 'data' in resp2 and 'token' in resp2['data']:
            token = resp2['data']['token']

        if not token:
            raise Exception(f"Login shayad fail ho gaya. Raw Response: {resp2}")
        ...
        await users_col.update_one({"id": current_user["id"]}, {"$set": {
            ...
            "kotak_bearer_token": token # 💾 MONGODB MEIN SAVE
        }})
...
async def connect_kotak_quick(req: KotakTotpOnly, current_user: dict = Depends(get_current_user)):
    try:
        ...
        await users_col.update_one({"id": current_user["id"]}, {"$set": {
            "kotak_bearer_token": token # 💾 MONGODB MEIN UPDATE
        }})
